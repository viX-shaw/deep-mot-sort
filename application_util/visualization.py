...
class Visualization(object):
    ...
    def __init__(self, seq_info, update_ms):
        ...
        self.last_idx = seq_info["max_frame_idx"]
        self.track_lines = {}
        print("max_frame_index", self.last_idx)
    ...
    def draw_trackers(self, tracks):
        ...
        for track in tracks:
            d = track.to_tlwh()
            d=[d[0], d[1],d[0]+d[2],d[1]+d[3]]
            d.append(track.track_id)
            thisLine=[]
            trackerDot = [[int((d[0] + (d[2]-d[0])/2)),int((d[3]))]]
            if str(d[4]) in self.track_lines:
                thisLine = self.track_lines[str(d[4])]
                thisLine = np.append(thisLine,trackerDot,axis=0)

                MAX_DOTS=16
                if thisLine.shape[0] > MAX_DOTS :
                    thisLine = thisLine[ (thisLine.shape[0]-MAX_DOTS):]
